...
app.use(function(req, res, next) {
  res.header("Access-Control-Allow-Origin", "http://s.codepen.io");
...
server.on('upgrade', function (req, socket, body) {
  ...
  if(route[route.length-1][0] === '?'){
    route[route.length-1] = route[route.length-1].substr(1)
    params.query = qp.toObject(route[route.length-1])
  }
